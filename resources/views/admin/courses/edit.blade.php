    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course') ? 'is-invalid' : '' }}" type="text" name="course" id="course" value="{{ old('course', $course->course) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('courseid') ? 'is-invalid' : '' }}" type="number" name="courseid" id="courseid" value="{{ old('courseid', $course->courseid) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_of_trainees') ? 'is-invalid' : '' }}" type="number" name="number_of_trainees" id="number_of_trainees" value="{{ old('number_of_trainees', $course->number_of_trainees) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('department') ? 'is-invalid' : '' }}" name="department" id="department">
                    ...
                    @foreach(App\Models\Course::DEPARTMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('department', $course->department) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
