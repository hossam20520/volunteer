    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.registereds.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('branch_code') ? 'is-invalid' : '' }}" type="text" name="branch_code" id="branch_code" value="{{ old('branch_code', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('joined_date') ? 'is-invalid' : '' }}" type="text" name="joined_date" id="joined_date" value="{{ old('joined_date') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('have_account') ? 'is-invalid' : '' }}" name="have_account" id="have_account">
                    ...
                    @foreach(App\Models\Registered::HAVE_ACCOUNT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('have_account', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
